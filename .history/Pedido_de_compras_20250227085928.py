...
# Ignorar o cabeçalho e verificar as colunas
# As colunas do cabeçalho são mantidas por padrão, então vamos trabalhar com as linhas a partir de 1
for col in df.columns:
    if (df[col].iloc[1:] == 0).all():  # Verifica se todas as linhas (exceto o cabeçalho) têm valor 0
        df.drop(columns=[col], inplace=True)  # Exclui a coluna
# Salvar o resultado em uma nova planilha
df.to_excel('caminho/para/salvar/nova_planilha.xlsx', index=False)