...
with open(caminho_planilha, 'rb') as anexo:
...
# Anexando a planilha de CPs com erro de cadastro
with open(caminho_planilhaErro, 'rb') as anexo:
    part = MIMEBase('application', 'octet-stream')
    part.set_payload(anexo.read())
    encoders.encode_base64(part)  # Codifica o arquivo em base64
    part.add_header(
        'Content-Disposition',
        f'attachment; filename="CPs com erro de cadastro.xlsx"'
    )
    msg.attach(part)
