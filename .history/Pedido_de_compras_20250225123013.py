...
# Iterar pelas colunas da última para a primeira (importante para não afetar o índice das colunas ao excluir)
for col in range(ws.max_column, 0, -1):  # Percorrendo de trás para frente      
    todos_zero = True  # Assume que todos os valores são zero
     # Iterar pelas células de uma coluna
    for row in range(1, ws.max_row + 1):
        valor = ws.cell(row=row, column=col).value
        if valor is not None and valor != 0 and valor != "0" and valor != 0.0:
            todos_zero = False  # A coluna não é completamente zero
            break  # Pode parar de verificar esta coluna
    # Se todos os valores da coluna forem zero, exclui a coluna
    if todos_zero: 
        ws.delete_cols(col)   
