...
# Limpar os espaços no começo e no final dos valores de todas as colunas de texto
for coluna in df.select_dtypes(include=['object']).columns:
    df[coluna] = df[coluna].str.strip()  # Remove espaços à esquerda e à direita

# Verificar as colunas onde todos os valores (não nulos) são iguais a zero
colunas_para_excluir = []
for coluna in df.columns:
    # Verifica se todos os valores não nulos na coluna são zero
    if np.all(df[coluna].fillna(0) == 0):  # Preenche valores nulos com 0 para a verificação
