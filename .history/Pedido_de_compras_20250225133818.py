...
# Verificar colunas onde todas as células são iguais a zero
colunas_para_excluir = []
for coluna in df.columns:
    # Verifica se todos os valores na coluna são zero (ignorando valores nulos)
    if np.all(df[coluna].fillna(0) == 0):
        colunas_para_excluir.append(coluna)

# Excluir as colunas identificadas
df = df.drop(columns=colunas_para_excluir)

# Salvar a planilha modificada (opcional)
